...
class Xlsx:
        ...
        if filepath:
            ...
            if len(self.wb.sheetnames) == 1:
                ...
                if sheetname:
                    ...
                    # Display availible sheets and set worksheet based on selection
        ...
        try:
            for row, _cell in enumerate(other.ws['A'], 1):
                for scol, tcol in columns.items():
                    self.ws[f'{tcol.upper()}{row}'] = other.ws[f'{scol.upper()}{row}'].value
    ...
    def sort_and_replace(self, sortcol, startrow=1) -> object:
        ...
        try:
            ...
            for sortval, rowvalues in sorted(sortme):
                self.ws.append(rowvalues)
                    ...
                    if cell.value and str(cell.value).lower() not in skip:
                        for find, replace in fndrplc.items():
                            if find in str(cell.value):
                                self.ws[f'{col.upper()}{row}'] = cell.value.replace(
                                    find, replace)
                ...
                if cell.value and row >= startrow:
                    for item in vals:
                        if item in str(cell.value):
                            ...
                            self.ws[f'{scol.upper()}{row}'] = cell.value.replace(
                                item, '')
                            break
    ...
    def set_cell_size(self, pairs) -> object:
        ...
        to adjust the size of cells from each pair. If dict key is type: str, 
        adjusts column width. If dict key is type: int, adjusts row height. 
